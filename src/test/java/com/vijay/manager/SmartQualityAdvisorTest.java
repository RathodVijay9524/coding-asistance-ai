...
class SmartQualityAdvisorTest {

    private OpenAiChatModel chatModel;
    private SmartQualityAdvisor advisor;

    @BeforeEach
    void setUp() {
        chatModel = Mockito.mock(OpenAiChatModel.class);
        advisor = new SmartQualityAdvisor(chatModel);
    }

    @Test
    @DisplayName("adviseCall should analyze quality and return original response")
    void adviseCall_analyzesAndReturns() {
        ...
        when(userMessage.getText()).thenReturn("Explain the role of KnowledgeGraphAdvisor in this system.");
        when(chain.nextCall(request)).thenReturn(response);
        when(response.chatResponse().getResult().getOutput().getText())
                .thenReturn("KnowledgeGraphAdvisor detects concepts like weather, calendar, and search to enrich context.");
